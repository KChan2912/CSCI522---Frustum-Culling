...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
	{
		...
		if (!pEvt->isInstanceOf < Events::Event_GATHER_DRAWCALLS>())
		{
			OutputDebugStringA("DRAWWING BB: EVT IS NOT OF GATHER DRAWCALLS");
			continue;
		}
		...
		WorldTrans.loadIdentity();
		if (hparentSN.isValid())
			WorldTrans = hparentSN.getObject<SceneNode>()->m_worldTransform;
		//DEBUG DRAW
		//if(!pInst->m_culledOut)
			//pInst->DrawBB(WorldTrans);
	}
    ...
    if (pMeshCaller->m_performBoundingVolumeCulling)
    {
        ...
        for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
			...
			for (int i = 0; i < 8; i++)
			{
				if (pInst->aabbCorners.m_size > 0) {
				
					if (((WorldTrans * pInst->aabbCorners[i]) - pcam->pos).dotProduct(pcam->TopPlane) < 0
						&& ((WorldTrans * pInst->aabbCorners[i]) - pcam->pos).dotProduct(pcam->LeftPlane) < 0
						&& ((WorldTrans * pInst->aabbCorners[i]) - pcam->pos).dotProduct(pcam->RightPlane) < 0
						&& ((WorldTrans * pInst->aabbCorners[i]) - pcam->pos).dotProduct(pcam->BotPlane) < 0
						&& ((WorldTrans * pInst->aabbCorners[i]) - pcam->nearCenter).dotProduct(pcam->NearPlane) > 0
						&& ((WorldTrans * pInst->aabbCorners[i]) - pcam->farCenter).dotProduct(pcam->FarPlane) > 0)
					{
						counter++;
					}
